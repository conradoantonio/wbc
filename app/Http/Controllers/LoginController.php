...
class LoginController extends Controller
{
    ...
    public function index(Request $req)
    {
        if ( auth()->attempt(['email' => $req->email, 'password' => $req->password]) ) {
            ...
            } elseif( auth()->user()->role->name == 'App' && auth()->user()->verificado == 1 && auth()->user()->tipo_login_id == 1 ) {
                    
                return redirect()->to('mi-perfil');

            } elseif( auth()->user()->role->name == 'Alternos' ) {
    ...
    public function loadDashboard()
    {
        ...
        // $graphic_data = $this->getWeeklySales();
        ...
        return view('layouts.dashboard', ['data' => json_decode($data), 'proximos' => $proximos, 'graphic_data' => [], 'title' => $title, 'menu' => $menu]);
        // return view('layouts.dashboard', ['data' => json_decode([]), 'proximos' => [], 'graphic_data' => [], 'title' => $title, 'menu' => $menu]);
    }
    ...
    public function getDashboarData() 
    {
        ...
        #Total ingresos
        $data->totalIngresos = Payment::sum('amount');

        #Total de eventos
        $data->totalPropiedades = Property::count();

        #Total de usuarios registrados
        $data->totalUsuarios = User::whereIn('role_id', [2])->count();

        #Total de usuarios registrados
        $data->totalUsuariosNuevos = User::whereIn('role_id', [2])->where('created_at', '>=', ( date('Y-m').'-01' ) )->count();

        #Total de eventos vigentes
        $data->totalBlogs = Blog::count();

        #Total de visitas históricas
        // $data->totalEventosHoy = Evento::whereHas('fechas', function($query){
        //     $query->where('fecha', date('Y-m-d'));
        // })->count();

        #Día actual
        // $dia_actual = utf8_encode(strftime('%w', strtotime($this->actual_date)));
